 ...
 * 지도에 올릴 제보만 남긴다.
 *
 * 서버가 `live=true` 로 걸러 주더라도 앱에서 한 번 더 본다. 검토 전(`PENDING`)·
 * 반려(`REJECTED`)·신고 누적(`HIDDEN`) 제보가 섞여 내려오면 검토 절차가
 * 무의미해지기 때문이다. 끝난 제보도 여기서 뺀다.
 */
export function visibleReports(
  ...
  return reports.filter(
    (report) => report.status === 'ACTIVE' && new Date(report.endsAt).getTime() > now,
  )
}
